...
export default function RetakeList({
  ...
  const renderDropdownMenu = (retake: Retake) => (
    <>
          ...
          <div
            ...
            <div className="mb-spacing-300 flex items-start justify-between">
              <button
                ...
                <div className="font-medium text-body text-content-standard-primary">{retake.student.name}</div>
              ...
              <div className="relative">
                <button
                  ...
                  className="rounded-radius-200 px-spacing-200 py-spacing-100 transition-colors hover:bg-components-fill-standard-secondary">
                  <svg className="h-5 w-5 text-content-standard-tertiary" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z" />
                  </svg>
                </button>
                {openMenuId === retake.id && renderDropdownMenu(retake)}
              </div>
            </div>

            <div className="mb-spacing-300 space-y-spacing-100">
            ...
            <div className="mb-spacing-300 flex flex-wrap items-center gap-spacing-200">
              {getStatusBadge(retake.status)}
              <button onClick={() => onManagementStatusChange(retake)} className="transition-opacity hover:opacity-70">
                {getManagementStatusBadge(retake.management_status)}
              </button>
            </div>

            <div className="flex flex-wrap items-center gap-spacing-300 text-content-standard-secondary text-label">
              <span>
                예정일:{" "}
                {retake.current_scheduled_date || <span className="text-content-standard-tertiary">미지정</span>}
              </span>
              {retake.postpone_count > 0 && <span>연기 {retake.postpone_count}회</span>}
              {retake.absent_count > 0 && <span>결석 {retake.absent_count}회</span>}
            </div>
          </div>
        ))}
      </div>
