...
const getEventClass = (event: CalendarEvent): string => {
  switch (event.type) {
    ...
    case "retake":
      if (event.metadata?.status === "completed") return "bg-success/10 text-success";
      if (event.metadata?.status === "absent") return NEUTRAL;
      return "bg-destructive/10 text-destructive";
    case "clinic":
      if (event.metadata?.status === "attended") return "bg-success/10 text-success";
      if (event.metadata?.status === "absent") return NEUTRAL;
      return "bg-event-clinic/10 text-event-clinic";
    case "assignment":
      if (event.metadata?.status === "completed") return "bg-success/10 text-success";
...
export default function CalendarEventItem({ event, onClick }: Props) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "w-full truncate rounded-md px-2 py-0.5 text-left text-xs transition-transform duration-150 hover:scale-[1.02]",
        getEventClass(event),
      )}>
      {event.title}
    </button>
