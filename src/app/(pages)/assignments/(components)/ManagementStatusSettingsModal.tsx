...
import {
  useAssignmentManagementStatusCreate,
  useAssignmentManagementStatusDelete,
  useAssignmentManagementStatuses,
  useAssignmentManagementStatusReorder,
  useAssignmentManagementStatusUpdate,
} from "../(hooks)/useAssignmentManagementStatuses";
...
export default function ManagementStatusSettingsModal() {
  ...
  const { statuses, isLoading } = useAssignmentManagementStatuses();
  const { mutateAsync: createStatus, isPending: isCreating } = useAssignmentManagementStatusCreate();
  const { mutateAsync: updateStatus } = useAssignmentManagementStatusUpdate();
  const { mutateAsync: deleteStatus } = useAssignmentManagementStatusDelete();
  const { mutateAsync: reorderStatuses } = useAssignmentManagementStatusReorder();
  ...
  return (
    <Modal
      ...
      title="과제 관리 상태 설정"
      subtitle="과제 관리 상태 목록을 추가, 수정, 삭제하고 순서를 변경할 수 있습니다"
      footer={
