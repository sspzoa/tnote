  ...
  return (
    <div className="border-border border-b">
      <button
        ...
        className="flex w-full min-w-0 flex-col gap-1 px-5 py-4 text-left transition-colors duration-150 hover:bg-muted/40">
        <div className="flex w-full items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            <span className="truncate font-semibold text-foreground text-sm">{title}</span>
            <span
              className={cn(
                "shrink-0 rounded-md px-1.5 py-0.5 font-semibold text-[10px] leading-4",
                getMessageTypeBadgeStyle(batch.message_type),
              )}>
              {getMessageTypeLabel(batch.message_type)}
            </span>
            {batch.fail_count > 0 && (
              <span className="shrink-0 rounded-md bg-destructive-soft px-1.5 py-0.5 font-semibold text-[10px] text-destructive leading-4">
        ...
        <p className="line-clamp-2 text-muted-foreground text-sm">{batch.message_content}</p>
